...
def _calculate_dadt_ccd(msh, ccd_file, coil_matrix, didt, geo_fn):
    ...
    node_data = gmsh.NodeData(A)
    node_data.node_number = msh.nodes.node_number

    if geo_fn is not None:
        gmsh.write_geo_spheres(d_position, geo_fn,
...
def _calculate_dadt_nifti(msh, nifti_image, coil_matrix, didt, geo_fn):
    ...
    if isinstance(nifti_image, basestring):
    ...
    if geo_fn is not None:
        ...
        gmsh.write_geo_spheres(pos, geo_fn,
...
def set_up_tms(msh, coil_file, coil_matrix, didt=1e6, fn_out=None, fn_geo=None):
    ...
    didt(optional): float
        value of dI/dt
   fn_out(optional): str
        name of file to output. default: don't output any file
   fn_geo(optional): str
   ...
   Returns
   ---------
    msh_setup: simnibs.gmsh_numpy.Msh
        mesh with NodeData fields for set-up
    """
    new_msh = copy.deepcopy(msh)
    coil_matrix = np.array(coil_matrix, dtype=float)
    dadt = set_up_tms_dAdt(msh, coil_file, coil_matrix, didt, fn_geo)
    new_msh.elmdata = []
    new_msh.nodedata = [dadt]
    dadt.mesh = new_msh
    if fn_out is not None:
        new_msh.fn = fn_out
        gmsh.write_msh(new_msh, fn_out)
