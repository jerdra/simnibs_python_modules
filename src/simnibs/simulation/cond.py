...
def standard_cond():
    ...
    # Eye region (mix of mostly optic nerve & fat)
    S[8].name = 'Eye_region'
    S[8].value = 0.25
    S[8].descrip = 'mix of mostly optic nerve & fat (from Opitz, Paulus, Thielscher, submitted)'
...
def cond2elmdata(mesh, cond_list, anisotropy_volume=None, affine=None,
    ...
    if anisotropy_volume is None:
        cond = gmsh.ElementData(np.zeros(mesh.elm.nr, dtype=float),
    ...
    # Anisotropic
        ...
        cond = gmsh.ElementData.from_data_grid(
...
def TensorVisualization(cond, all_compoents=False):
    ...
    data = []
    data.append(
        gmsh.ElementData(eig_vec[:, :, 0] * eig_val[:, 0, None],
                         name='max_eig'))
    if all_compoents:
        data.append(
            gmsh.ElementData(eig_vec[:, :, 1] * eig_val[:, 1, None],
                             name='mid_eig'))
        data.append(
            gmsh.ElementData(eig_vec[:, :, 2] * eig_val[:, 2, None],
                             name='min_eig'))
