...
def _get_transform(center, y_axis, mesh, mesh_surface=[5, 1005], y_type='relative',
    ...
    mesh.assert_compact_element_ordering()
    mesh.assert_compact_node_ordering()
...
def _edge_list(triangles):
    ...
    method in gmsh_numpy '''
    edges = triangles[:, [[0, 1], [0, 2], [1, 2]]]
    edges = edges.reshape(-1, 2)
    hash_array = np.array([hash(f.tobytes()) for f in np.sort(edges, axis=1)])
...
def _triangle_with_points(points, triangles, nodes, eps=1e-5, edge_list=None):

    triangles = _fix_triangle_node_ordering(triangles, nodes)
    ...
    kdtree = scipy.spatial.cKDTree(tr_baricenters)
...
def _build_electrode_on_mesh(center, ydir, poly, h, mesh, el_vol_tag, el_surf_tag,
    ...
    mesh.nodes.node_number = 1 + np.arange(len(mesh.nodes.node_coord), dtype=int)
    ...
    mesh.elm.elm_number = 1 + np.arange(len(mesh.elm.tag1), dtype=int)
    return mesh
...
def put_electrode_on_mesh(elec, mesh, elec_tag, skin_tag=[5, 1005], extra_add=400,
    ...
    Parameters
    ---------------
    elec: simnibs.simulation.nnav.ELEC
        Structure with electrode information
    mesh: simnibs.msh.gmsh_numpy.mesh
    ...
    plug_add:
        Number to be added to designate plug surfaces. Default:2electrodi000

    Returns:
    ---------
    mesh_w_electrode: simnibs.msh.gmsh_numpy
