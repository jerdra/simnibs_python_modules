...
'''
    ...
    Copyright (C) 2018 Kristoffer H Madsen, Guilherme B Saturnino, Axel Thielscher
...
def volumetric_affine(image, affine, target_space_affine,
    ...
    outdim = xyzvox.shape[1:]
    xyzvox = xyzvox.reshape(3, -1)
    # Bring points from the grid in target space to x, y,z in target space
    # Apply the affine transformation to bring points to the original space
    # Transform points from x, y, z in the original space to voxel coordinates
    iM = np.linalg.inv(im_affine).dot(affine.dot(target_space_affine))
    coords = iM[:3, :3].dot(xyzvox) + iM[:3, 3, None]

    outdata = _interpolate(im_data, coords, intorder, outdim, cpus)
...
def get_names_from_folder_structure(m2m_folder):
    ...
    if not os.path.isfile(ref_mni):
        warnings.warn('Could not find reference volume in mni space at: {0}'.format(
                ref_mni))

    ref_conf = os.path.abspath(os.path.join(
        m2m_folder, 'T1fs_conform.nii.gz'))
    if not os.path.isfile(ref_conf):
        warnings.warn('Could not find reference volume in subject space at: {0}'.format(
                ref_conf))
    ...
    try:
        ...
        raise IOError('Could not find subject ID from path')

    mesh = os.path.normpath(os.path.join(
        m2m_folder, '..', subid + '.msh'))
    if not os.path.isfile(mesh):
        warnings.warn('Could not find head mesh at: {0}'.format(mesh))

    mni2conf_nonl = os.path.abspath(os.path.join(
        m2m_folder, 'toMNI', 'MNI2Conform_nonl.nii'))

    if not os.path.isfile(mni2conf_nonl):
        if os.path.isfile(mni2conf_nonl + '.gz'):
            mni2conf_nonl = mni2conf_nonl + '.gz'
        else:
            warnings.warn('Could not find MNI2Conform non-linear transform at: {0}'.format(
                    mni2conf_nonl))

    conf2mni_nonl = os.path.abspath(os.path.join(
        m2m_folder, 'toMNI', 'Conform2MNI_nonl.nii'))

    if not os.path.isfile(conf2mni_nonl):
        if os.path.isfile(conf2mni_nonl + '.gz'):
            conf2mni_nonl = conf2mni_nonl + '.gz'
        else:
            warnings.warn('Could not find Conform2MNI non-linear transform at: {0}'.format(
                    conf2mni_nonl))

    mni2conf_6dof = os.path.abspath(os.path.join(
        m2m_folder, 'toMNI', 'MNI2conform_6DOF'))

    if os.path.isfile(mni2conf_6dof + '.txt'):
        mni2conf_6dof = mni2conf_6dof + '.txt'
    elif os.path.isfile(mni2conf_6dof + '.mat'):
        mni2conf_6dof = mni2conf_6dof + '.mat'
    else:
        warnings.warn('Could not find MNI2Conform 6DOF transform at: {0}'.format(
                mni2conf_6dof))

    mni2conf_12dof = os.path.abspath(os.path.join(
        m2m_folder, 'toMNI', 'MNI2conform_12DOF'))
    if os.path.isfile(mni2conf_12dof + '.txt'):
        mni2conf_12dof = mni2conf_12dof + '.txt'
    elif os.path.isfile(mni2conf_12dof + '.mat'):
        mni2conf_12dof = mni2conf_12dof + '.mat'
    else:
        warnings.warn('Could not find MNI2Conform 12DOF transform at: {0}'.format(
                mni2conf_12dof))
    names = {
        'mesh': mesh,
        'reference_mni': ref_mni,
...
def warp_volume(image_fn, m2m_folder, out_name,
    ...
    '''
    from .gmsh_numpy import read_msh
...
def interpolate_to_volume(fn_mesh, reference, fn_out, create_masks=False,
    ...
    continuous: 'auto' or bool
        ...
        True for normJ and J, and False for other fields. Default: auto
    '''
    from .gmsh_numpy import read_msh, ElementData
...
def get_surface_names_from_folder_structure(m2m_folder):
    ...
    def look_up(d, fn):
        ...
        if os.path.isfile(f):
            ...
            raise IOError(
             'Could not find file: {0} in directory {1}'.format(d, fn))

    headreco_suf_dir = os.path.normpath(
        os.path.join(d, 'segment', 'cat', 'surf'))
    mri2mesh_surf_dir = os.path.normpath(
        os.path.join(d, '..', 'fs_' + subid, 'surf'))
    names = {}
    if os.path.isdir(headreco_suf_dir):
        segtype = 'headreco'
        names['surf_dir'] = headreco_suf_dir
        names['lh_midgm'] = look_up(headreco_suf_dir, 'lh.central.T1fs_conform.gii')
        names['rh_midgm'] = look_up(headreco_suf_dir, 'rh.central.T1fs_conform.gii')
        names['ref_fs'] = look_up(m2m_folder, 'ref_FS.nii.gz')

    elif os.path.isdir(mri2mesh_surf_dir):
        segtype = 'mri2mesh'
        names['subj_id'] = subid
        names['subj_dir'] = os.path.normpath(os.path.join(d, '..'))
        names['surf_dir'] = headreco_suf_dir
        names['lh_gm'] = look_up(mri2mesh_surf_dir, 'lh.pial')
        names['lh_wm'] = look_up(mri2mesh_surf_dir, 'lh.white')
        names['rh_gm'] = look_up(mri2mesh_surf_dir, 'rh.pial')
        names['rh_wm'] = look_up(mri2mesh_surf_dir, 'rh.white')
        names['ref_fs'] = look_up(m2m_folder, 'ref_FS.nii.gz')

    else:
        raise IOError('Could not find surface files in m2m folder. SPM-only segmentation?')

    return names, segtype
...
def middle_gm_interpolation(mesh_fn, m2m_folder, out_folder, out_fsaverage=None,
    ...
    '''
    #from .gmsh_numpy import read_freesurfer_surface, read_gifti_surface, _middle_surface
    from . import gmsh_numpy as gmsh
    ...
    if segtype == 'mri2mesh':
        ext = ''
        for hemi in ['lh', 'rh']:
            wm_surface = gmsh.read_freesurfer_surface(names[hemi + '_wm'])
            gm_surface = gmsh.read_freesurfer_surface(names[hemi + '_gm'])
            middle_surf[hemi] = gmsh._middle_surface(wm_surface, gm_surface, depth)
            gmsh.write_freesurfer_surface(middle_surf[hemi],
                                          os.path.join(out_folder, hemi + '.central'),
                                          names['ref_fs'])
    elif segtype == 'headreco':
        ext = ''
        for hemi in ['lh', 'rh']:
            middle_surf[hemi] = gmsh.read_gifti_surface(names[hemi + '_midgm'])
            gmsh.write_freesurfer_surface(middle_surf[hemi],
                                          os.path.join(out_folder, hemi + '.central'),
                                          names['ref_fs'])
    names_subj = []
    names_fsavg = []
    h = []
    for name, data in m.field.iteritems():
        for hemi in ['lh', 'rh']:
            if fields is None or name in fields:
                ...
                if data.nr_comp == 3:
                    ...
                    for q_name, q_data in q.iteritems():
                        out_subj = os.path.join(
                            out_folder, hemi + sim_name + '.central.' + name + '.' + q_name)
                        gmsh.write_curv(out_subj,
                                        q_data.value,
                                        middle_surf[hemi].elm.nr)
                        names_subj.append(out_subj)
                        h.append(hemi)
                        if out_fsaverage is not None:
                            names_fsavg.append(os.path.join(
                                out_fsaverage, hemi + sim_name + '.fsavg.' + name + '.' + q_name +
                                ext))
                    ...
                    if field_name in m.field.keys() and q_name in quantities:
                        ...
                        if out_fsaverage is not None:
                            names_fsavg.append(os.path.join(
                                    out_fsaverage, hemi + sim_name + '.fsavg.' + name + ext))

    if out_fsaverage is not None:
        if segtype == 'mri2mesh':
            for sub, fs, he in zip(names_subj, names_fsavg, h):
                run_mris_preproc(names['subj_id'], he, fs,
                                 sub, names['subj_dir'])

        elif segtype == 'headreco':
            script_fn = os.path.join(out_fsaverage, 'to_fsavg.m')
            with open(script_fn, 'w') as f:
                f.write("addpath('{0}', '{1}');\n".format(
                    os.path.dirname(os.path.abspath(__file__)),
                    os.path.join(SIMNIBSDIR, 'resources', 'spm12')))
                f.write("map_results_to_fsaverage(...\n")
                f.write("{{{0}}},...\n".format(
                    ',...\n'.join(["'{}'".format(n) for n in names_subj])))
                f.write("{{{0}}},...\n".format(
                    ',...\n'.join(["'{}'".format(n) for n in names_fsavg])))
                f.write("'{}');\n".format(m2m_folder))
            cmd = ["matlab", "-nosplash", "-nodesktop", "-wait",
                   "-r \"try,run('{}');catch ME,rethrow(ME);end,quit;\"".format(script_fn)]
            run_command(cmd, realtime_output=True)
