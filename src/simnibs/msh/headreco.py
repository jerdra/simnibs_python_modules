...
from simnibs.msh import gmsh_numpy
...
def headmodel(argv):    
    ...
    path2mfiles = [os.path.join(SIMNIBSDIR,"Python_modules","src","simnibs","msh"),os.path.join(SIMNIBSDIR,"resources","spm12")]
    ...
    if args.mode in ["all","preparevols"]:
        ...
        for k in tspm.iterkeys():
            tspm[k] = os.path.join(spm,"w"+k)
        if docat:
            for k in tcat.iterkeys():
    ...
    if args.mode in ["all","cleanvols"]:
        ...
        vals = {
            ...
            "air_raw"   : sgm_files[5],
            "out_dir"   : mask_prep,
            "prior_tissue" : [v for k,v in tspm.iteritems() if "tissue" in k][0],
            "prior_eye1" : [v for k,v in tspm.iteritems() if "eye1" in k][0],
            "prior_eye2" : [v for k,v in tspm.iteritems() if "eye2" in k][0],
            "prior_air" : [v for k,v in tspm.iteritems() if "air" in k][0],
            "prior_spinal" : [v for k,v in tspm.iteritems() if "spinal" in k][0],
            "prior_ventricles_lateral" : [v for k,v in tspm.iteritems() if "ventricles" in k][0]
            }
    ...
    if args.mode in ["all","volumemesh"]:
        ...
        gmsh_numpy.write_msh(vmsh, final_mesh)
        remove_files(ovmsh)
        ...
        for k in [1, 2]:
           ...
           ed = gmsh_numpy.ElementData(field)
           ed.mesh = vmsh
...
def make_surface_meshes(input_files,out_dir,temp_dir,vertex_density=0.5,
    ...
    if catsurf or mask.has_key(key):
...
def make_volume_mesh(subject_id,input_files,out_dir):
    ...
    files = {}
    files = files.fromkeys(["wm", "gm", "csf", "bone", "skin", "eyes",
...
def visualize(subject_id,surface_meshes,T1,control_mask_initial,
    ...
    def combine_meshes_fsmesh(surf_a, surf_b, out):
        s_a = gmsh_numpy.read_stl(surf_a)
        s_b = gmsh_numpy.read_stl(surf_b)
        c = s_a.join_mesh(s_b)
        gmsh_numpy.write_freesurfer_surface(c, out, ref_fs=ref_fs)
...
def parse_args(argv):
    ...
    cat = {
        "action"  : "store_true",
        "default" : False,
        "help"    : """Use CAT12 for brain segmentation (default: %(default)s)."""
        }
    biasreg = {
        "type"    : float,
        "default" : 0.01,
    ...
    # optional
    parser_prepvols.add_argument("--cat", **cat)
    ...
    parser_prepcat.add_argument("--cat", **cat)
    # positional
    ...
    # optional
    parser_cleanvols.add_argument("--cat", **cat)
    ...
    # optional
    parser_surfmesh.add_argument("--cat", **cat)
    ...
    # optional
    parser_volmesh.add_argument("--cat", **cat)
