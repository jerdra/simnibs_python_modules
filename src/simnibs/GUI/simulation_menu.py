...
class SimulationOptionsDialog (QtWidgets.QDialog):
    ...
    def selectOtherOptions(self):
        ...
        '''
        keepall_cb = QtWidgets.QCheckBox()
        keepall_cb.setText('keep all files')
        if self.options.keepall: keepall_cb.toggle()
        keepall_cb.toggled.connect(self.changeOptions)
        layout.addWidget(keepall_cb,0 , QtCore.Qt.Alignment(1))
        '''
        ...
        #options_box.keepall_cb = keepall_cb
    ...
    def changeOptions(self):
        #self.options.keepall = self.options_box.keepall_cb.isChecked()
