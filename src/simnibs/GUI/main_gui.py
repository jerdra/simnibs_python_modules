...
simnibs_version = str(version.__version__)
...
class TDCS_GUI(QtWidgets.QMainWindow):
    ...
    def m2mFolderDialog(self):
        ...
        if ' ' in folder:
            ...
            fn_mesh = nnav.get_mesh_name_from_m2m_folder(folder)
    ...
    def loadHeadModel(self, fn):
        ...
        if not self.session.subpath:
            m2m = nnav.get_m2m_folder_from_mesh_name(fn)
            if m2m is not None:
                self.m2m_folder_lineEdit.setText(m2m)
                self.session.subpath = m2m

        if not self.session.eeg_cap:
            m2m = self.session.subpath
            if m2m and os.path.isdir(m2m):
                self.session.eeg_cap = nnav.get_eeg_cap_from_m2m_folder(m2m)

        if not self.session.pathfem:
            pathfem = os.path.join(os.path.split(fn)[0], 'simnibs_simulation')
    ...
    '<p><center>      version' + simnibs_version+'      </center></p>'+\
    '<p><center>Simulation of eletromagnetic fields generated by tDCS and TMS</center></p>' +\
    '<p><center>Copyright (C) 2018 Andre Antunes, Guilherme B Saturnino, Axel Thielscher,Kristoffer H Madsen </center></p>'+\
    ...
    def lookForTensors(self, fn):
        ...
        m2m = nnav.get_m2m_folder_from_mesh_name(fn)
        if m2m is not None:
            fname_tensor = nnav.get_dir_tensor_from_m2m_folder(m2m)
    ...
    def openSimnibsFile(self, fn=None):
        ...
        S = nnav.import_struct(file_full_path)
        self.session = S
    ...
    #Generates a nnav.session() structure, for saving and running
    ...
    def saveSimFile(self):
        ...
        nnav.save_matlab_nnav(S, full_path)

        print "file ", full_path, " saved "
    ...
    def runSimnibs(self):
        ...
        #Looks if a simulation was already run in the folder
        #Does not allow the user to run 2 simulations in the same folder
        #if S.pathfem == '':
        #    S.pathfem = S.subpath + '/fem/'
        '''
        if os.path.isdir(S.path_fem):
            filenames = os.listdir(S.path_fem)
            for filename in filenames:
                if filename.endswith(".simnibs"):
                    message = "A simulation was previously run on this directory with ID: "+ os.path.splitext(filename)[0] + '\n\n'
                    message += "To rerun the simulation, you need to either:\n"
                    message += "Delete the file "+ os.path.join(S.path_fem,filename) + '\n\n'
                    message += "Delete the directory " +  S.path_fem + '\n\n'
                    message += "Change Output Folder\n\n"
                    message += "To resume the simulation, type in the command line:\n"
                    message += "simnibs " + os.path.join(S.path_fem,filename)
                    QtWidgets.QMessageBox.critical(self, "Warning", message)
                    return None
        '''

        self.simThreads.append(runSimThread(sim_fn))
        ...
        self.progressScreens[-1].terminate_signal.connect(self.simThreads[-1].setStop)
...
class ElcTable(QtWidgets.QWidget):
    ...
    def writeOnPosColumn(self, row, pos):
        if pos is not None and len(pos) == 3:
             pos_text = str([round(n,4) for n in pos]).translate(None, "[]() ")
    ...
    def deleteElectrode(self):
        self.table_rows[self.currentRow()].electrode = nnav.ELECTRODE()
...
class COIL_TABLE_ROW:
    def __init__(self):
        #self.position_struc = nnav.POSITION()
        self.p1 = []
        self.p2 = []
...
class CoilTable (QtWidgets.QWidget):
    ...
    def returnCoilAndConds(self):
        self.tmslist.pos = []
        for row in self.table_rows:
            self.tmslist.pos.append(nnav.POSITION())
            if len(row.p1) == 3:
...
class ConductivitiesGui (QtWidgets.QDialog):
    ...
    def reset(self):
        ...
        self.simulist.aniso_maxratio = nnav.SimuList().aniso_maxratio
        self.simulist.aniso_maxcond = nnav.SimuList().aniso_maxcond
        self.simulist.anisotropy_type = nnav.SimuList().anisotropy_type
...
class runSimThread(QtCore.QThread):
    ...
    def __init__(self, simnibs_fn) :
        QtCore.QThread.__init__(self)
        self.sim_fn = simnibs_fn
        self.stop = False
    ...
    def run(self):
        ...
        simnibs_bin = os.path.join(SIMNIBSDIR,
                                   'bin',
                                   'simnibs')
        cmd = simnibs_bin + ' ' + self.sim_fn
