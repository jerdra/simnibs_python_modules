...
def sensor_setup(nr_theta=4, nr_phi=4, r_min=0.1, r_max=1, nr_r=5):
...
if __name__ == '__main__':
    """
    sensors = sensor_setup(nr_theta=20, nr_phi=20, r_min=0.1, r_max=1, nr_r=20)
    cathode = [2, 0, 0]
    anode = [-2, 0, 0]
    ...
    field_visualization.view_scalar_field_in_volume(sensors, pot)
    """
    import simnibs.msh.field_visualization as field_visualization
    import simnibs.msh.gmsh_numpy as gmsh
    """
    msh = gmsh.read_msh(os.path.join(
                        os.path.dirname(os.path.realpath(__file__)),
                        '..', 'testing_files', 'sphere3.msh'))

    msh = msh.crop_mesh(1005)
    dipole_pos = [0, 0, 90]
    dipole_moment = [0, 0, 0]

    v = sphere.potential_homogeneous_dipole(95, 1., dipole_pos, dipole_moment,
                                            msh.nodes.node_coord)

    field_visualization.view_scalar_field_in_surf(msh, 1005, v)
    """
    """

    sensors = sensor_setup(20, 20, 1.2, 2, 20)

    x = np.linspace(-0.8, 0.8, 15)
    y = np.linspace(-0.8, 0.8, 15)
    z = np.linspace(1.2, 1.5, 5)
    grid = np.meshgrid(x, y, z)

    g = np.concatenate([g[..., None] for g in grid], axis=3)
    points = g.flatten().reshape(-1, 3)
    points_outside = points[np.linalg.norm(points, axis=1) > 1]

    B = sphere.B_outside_sphere(1, [0, 0, 0.9], [1, 0, 0], points_outside)
    field_visualization.view_vectorial_field_in_volume(points_outside, B)
    """
    electrode_pos = sphere.fibonacci_sphere(31, R=95)
    from mayavi import mlab
    mlab.points3d(electrode_pos[:, 0], electrode_pos[:, 1], electrode_pos[:, 2],
